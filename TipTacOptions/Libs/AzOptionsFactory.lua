...
azof.objects.Slider = {
	...
	Init = function(self,option,cfgValue)
		...
		-- slider objects are reused, so always (re)apply the label's font size, optionally adjusted by option.fontSizeDelta
		local fontFace, fontSize, fontFlags = unpack(self.defaultLabelFont);
		self.text:SetFont(fontFace, fontSize + (option.fontSizeDelta or 0), fontFlags);
	...
	CreateNew = function(self)
		...
		f.defaultLabelFont = { f.text:GetFont() };
